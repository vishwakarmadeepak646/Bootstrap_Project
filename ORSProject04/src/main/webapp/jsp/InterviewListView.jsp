...
<%@page import="com.sunilos.p4.bean.InterviewBean"%>
	...
	<div class="card border-0 shadow-sm rounded-4 overflow-hidden">
		...
		<div
			...
			<h5 class="mb-0 fw-bold">
				<i class="bi bi-cart me-2"></i> INTERVIEW List
			</h5>
			<div class="d-flex gap-2">
				<a href="<%=ORSView.INTERVIEW_LIST_CTL%>" target="_blank"
					class="btn btn-sm btn-warning fw-semibold"> <i
					class="bi bi-file-earmark-pdf me-1"></i> Print PDF
				</a> <a href="<%=ORSView.INTERVIEW_LIST_CTL%>?type=doc" target="_blank"
					class="btn btn-sm btn-info fw-semibold"> <i
					class="bi bi-file-earmark-word me-1"></i> Print DOC
				</a> <a href="<%=ORSView.INTERVIEW_CTL%>"
					class="btn btn-sm btn-light text-primary fw-semibold"> <i
					class="bi bi-cart me-1"></i> Add Interview
				</a>
			</div>
		</div>

		<form action="<%=ORSView.INTERVIEW_LIST_CTL%>" method="POST">
			...
			<div
				...
				<input type="text" name="candidateName"
					...
					placeholder="Search by candidateName"
			...
			<div class="table-responsive">
				<table class="table table-hover align-middle mb-0">
					...
					<tbody>
						<%
						while (it.hasNext()) {
							InterviewBean bean = it.next();
						%>
						<tr>
							<td><input type="checkbox" name="ids"
								value="<%=bean.getId()%>"></td>
							<td class="text-muted small"><%=index++%></td>
							<td class="fw-semibold"><%=bean.getCandidateName() %></td>
							<td><%=bean.getInterviewerName()%></td>
							<td><%=bean.getResult()%></td>
							<td><%=bean.getFeedback()%></td>
							<td><a href="InterviewCtl?id=<%=bean.getId()%>"
